...
using CarpinteriaFrontend.cliente;
    ...
    public partial class Frm_Alta_Presupuesto : Form
    {
        ...
        public  Frm_Alta_Presupuesto(Accion modo, int nro)
        {
            ...
            if (modo.Equals(Accion.READ))
            {
               ...
               Cargar_presupuestoAsync(nro);
            }
        }

        private async Task Cargar_presupuestoAsync(int nro)
        {
             // servicio.ObtenerPresupuestoPorID(nro);
            string url = "https://localhost:44373/api/Presupuestos/" + nro.ToString();
            var resultado = await ClienteSingleton.GetInstancia().GetAsync(url);
            this.oPresupuesto = JsonConvert.DeserializeObject<Presupuesto>(resultado);
        ...
        private void gbDatosPresupuesto_Enter(object sender, EventArgs e)
        {
